...
# Import config_keys (reads from env vars)
from config_keys import (
    DEEPSEEK_API_KEY as CFG_DEEPSEEK_API_KEY,
    DEEPSEEK_API_URL as CFG_DEEPSEEK_API_URL,
    SILICONFLOW_API_KEY as CFG_SILICONFLOW_API_KEY
)

# Use config_keys values or env overrides
DEEPSEEK_API_KEY = os.environ.get("DEEPSEEK_API_KEY", CFG_DEEPSEEK_API_KEY)
DEEPSEEK_API_URL = os.environ.get("DEEPSEEK_API_URL", CFG_DEEPSEEK_API_URL)
SILICONFLOW_API_KEY = os.environ.get("SILICONFLOW_API_KEY", CFG_SILICONFLOW_API_KEY)
...
WATCHLIST_PATH = os.environ.get("WATCHLIST_PATH", str(BASE_DIR / 'watchlist.json'))
DATA_DIR = BASE_DIR / 'stockai_data'
DATA_DIR.mkdir(exist_ok=True)

# Try to import Futu-dependent modules gracefully
try:
    ...
    print(f"[Deploy] Futu SDK not available ({e}), running in cloud-only mode")
    StockAnalyzer = None
    IntradayAnalyzer = None
    FUTU_AVAILABLE = False

# Import other modules (should work without Futu)
...
if FUTU_AVAILABLE:
    try:
        ...
        print("✅ StockAnalyzer 初始化成功 (Futu)")
        print("✅ Multi-Model AI 分析器 初始化成功")
    except Exception as e:
        print(f"[Deploy] StockAnalyzer init failed (expected in cloud): {e}")
        analyzer = None
        intraday_analyzer = None
        multi_ai = None
else:
    print("ℹ️ Cloud mode: StockAnalyzer (Futu) disabled, limited cloud features available")
    try:
        multi_ai = MultiModelAIAnalyst()
        print("✅ Multi-Model AI 分析器 初始化成功")
    except Exception as e:
        print(f"⚠️ Multi-Model AI init: {e}")
...
def health_check():
    ...
    return jsonify({
        ...
        'service': 'StockAI',
        'version': '1.7',
        'futu_available': FUTU_AVAILABLE and analyzer is not None,
...
def get_hkex_verify(stock_code):
    ...
    try:
        ...
        # 检查本地数据文件状态
        local_data_dir = Path("C:/Users/MarcoMa/stockai_data/cache")
...
# 初始化基本面分析器
try:
    fundamental_analyzer = FundamentalAnalyzer(data_dir="C:/Users/MarcoMa/stockai_data/cache")
    print("✅ 基本面分析引擎 初始化成功")
except Exception as e:
    ...
    try:
        ...
        try:
            ...
            if fin_result and isinstance(fin_result, dict) and fin_result.get('success'):
                ...
                # 优先使用富途实时 PE（与 tradingview_demo 基本面分析一致）
...
if __name__ == '__main__':
    ...
    print(f"🚀 StockAI 智能分析系統啟動中... (Production Mode)")
    print(f"📡 Port: {port}, Debug: {debug}")
    print(f"🔑 Futu: {'✅' if FUTU_AVAILABLE else '❌ (cloud mode)'}")
